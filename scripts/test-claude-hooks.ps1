...
#   * The cmux wrapper finds it on PATH (skipping its own dir)
#   * Inside cmux, the wrapper injected --settings <hooks-json>
#   * Outside cmux, the wrapper is a transparent passthrough (no --settings)
#
# Separately we exercise `cmux hooks claude <event>` directly against a live
...
param(
    ...
    [string]$ShotPath = 'C:\tmp\cmux-claude-hooks.png'
)

$ErrorActionPreference = 'Stop'

# ---- 1. Fake-claude staging ----
$fakeDir = Join-Path $env:TEMP "cmux-fake-claude-$([Guid]::NewGuid().ToString('N').Substring(0,8))"
...
if (-not (Test-Path $cmuxExe)) { throw "cmux.exe not found at $cmuxExe" }

# ---- 2. Passthrough mode: no CMUX_PIPE → no --settings ----
$env:CMUX_PIPE = $null
$env:PATH = "$ToolsDir;$fakeDir;$env:PATH"
$out1 = & $cmuxExe wrap-claude --version test-arg 2>&1
...
if ($out1Joined -match '--settings') {
    throw "passthrough: --settings should NOT be injected outside cmux. Got: $out1Joined"
}
...
# ---- 3. Cmux-pane mode: CMUX_PIPE set → --settings <path> prepended ----
...
$env:CMUX_PIPE  = '\\.\pipe\cmux\fake-test'
$env:CMUX_PANE_ID = 'phase4-test'
$out2 = & $cmuxExe wrap-claude --version 2>&1
$out2Joined = $out2 -join "`n"
Write-Host ""
Write-Host "=== In-cmux run output ==="
Write-Host $out2Joined

if ($out2Joined -notmatch 'ARG: --settings') {
    throw "in-cmux: wrapper did not inject --settings. Got: $out2Joined"
}
...
if ($settingsIdx -lt 0 -or $settingsIdx -ge $lines2.Length - 1) {
    throw "in-cmux: couldn't find the --settings file-path argument"
}
$hooksFile = $lines2[$settingsIdx + 1]
if (-not (Test-Path $hooksFile)) {
    throw "in-cmux: --settings target '$hooksFile' does not exist"
}
...
function Send-HookEvent {
    ...
    $psi.EnvironmentVariables['CMUX_PIPE'] = $pipe
...
$statusCount = ($lines | Where-Object { $_ -match 'CmuxIpc\.recv.*type=status' }).Count
$notifyCount = ($lines | Where-Object { $_ -match 'CmuxIpc\.recv.*type=notify' }).Count
...
Write-Host "  - wrapper passthrough works outside cmux"
Write-Host "  - wrapper injects --settings JSON inside cmux"
