# cmux-win vs upstream cmux — parity tracker
...
Upstream cmux is positioned as a **parallel coding-agent workspace** — its
...
| Category | Upstream cmux | cmux-win | Status |
|---|---|---|---|
...
| Git branch awareness | Auto-detects branch per workspace | Branch chip exists, but pushed by agent via `cmux meta` — no auto-detect | 🟡 |
...
| Notifications | Native + in-app, severity levels | OSC 9 + `cmux:<level>` extension, sidebar pill, toast | ✅ |
| OSC 7 / cwd tracking | Standard | OSC 7 + Windows Terminal OSC 9;9 | ✅ |
| Settings / config | Repo-rooted `.cmux.json` per project + global | Global `settings.json` only | 🟡 |
...
2. **`cmux` CLI binary.** The IPC server already exists and matches
   upstream verbs — but every "agent integration" pattern starts with
   `cmux notify` / `cmux status`. A ~150-LOC .NET 8 AOT exe unlocks the
   ecosystem cheaply.
...
4. **Auto-detect git branch and listening ports per pane.** Today the
   chips only light up if an agent manually calls `cmux meta`. A polling
...
- Worktrees → `libgit2sharp` or shell out to `git.exe`; one worktree per
  run under `%LOCALAPPDATA%\cmux-win\worktrees\<run-id>`.
- `cmux` CLI → tiny .NET 8 native-AOT exe reading `CMUX_PIPE` /
  `CMUX_PANE_ID` (already injected by `Shell.BuildStartupCommandLine`).
