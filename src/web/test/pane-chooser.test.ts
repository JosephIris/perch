...
const CWD = "C:\\Users\\dev\\dev-projects\\perch";
...
test("folderName: last segment of a Windows path", () => {
  assert.equal(folderName("C:\\Users\\dev\\dev-projects\\perch"), "perch");
});
test("folderName: trailing separators are ignored", () => {
  assert.equal(folderName("C:\\Users\\dev\\dev-projects\\perch\\"), "perch");
});
test("folderName: unix path", () => {
  assert.equal(folderName("/home/dev/code/perch"), "perch");
});
...
test("option descriptions show the folder names", () => {
  ...
  assert.equal(descOf(opts[2]), "dev");    // default → default folder
});
