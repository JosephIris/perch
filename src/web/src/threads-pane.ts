// ThreadsPane = the right half of a project chat: one card per thread the
// coordinator has started, with its state, branch and the head of its last
// report. A card opens that thread's tab.
//
// It owns no data. Threads are ordinary sessions carrying `threadOf` (the
// project chat's id); the workspace hands this pane the ones that belong to
// its tab on every state push, so the panel can never disagree with the
// sidebar.
...
export class ThreadsPane {
  ...
  /** This project chat's threads, newest number last. */
  setThreads(threads: SessionView[]) {
    ...
    if (!sorted.length) {
      ...
      empty.appendChild(el("p", "threads__hint",
        "Tell the project chat what you need. It splits the work into threads, each a Claude in its own copy of the repo, and they show up here."));
  ...
  private card(t: SessionView): HTMLElement {
    ...
    // The report is Markdown; the card is a glance, so drop the emphasis marks.
