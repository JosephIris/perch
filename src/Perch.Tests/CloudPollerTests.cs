...
public class CloudPollerTests
{
    ...
    [
      {
        ...
        "labels": {
          "goog-dataproc-cluster-name": "batch-web-8f2c",
          "agent-owner": "joseph",
    ...
    private static CloudPoller Poller() => new()
    {
        ...
        LookupLedger = s => s == "dead-beef-session"
            ? new CloudLedger.Entry(s!, "build-sweep", "Sweep learning rates for v3", null, "9", 0, 0)
            : null,
    };
    ...
    public void Rolls_a_dataproc_cluster_up_into_one_row()
    {
        ...
        Assert.Equal("batch-web-8f2c", cluster.Name);
    ...
    {
        var got = Poller().Parse(Json);
        var gpu = got.Single(r => r.Name == "build-runner-h1");
        var cluster = got.Single(r => r.Kind == "cluster");
    ...
    public void An_orphan_still_knows_what_it_was_for()
    {
        ...
        var gpu = Poller().Parse(Json).Single(r => r.Name == "build-runner-h1");
        Assert.Equal("build-sweep", gpu.AgentName);
