...
public class ClaudeHeadlessTests
{
    ...
    public async Task RunAsync_FeedsStdin_StripsPaneEnv_AndParsesTheAnswer()
    {
        ...
        // A .cmd on Windows (the npm-installed shape, run through cmd.exe); a
        // shell script on macOS/Linux, where argv is bracketed per argument so
        // the empty `--tools ""` value is visibly preserved by .NET's
        // Arguments parsing.
        const string answer = "{\"type\":\"result\",\"subtype\":\"success\",\"result\":\"PONG\",\"total_cost_usd\":0.001,\"duration_ms\":5}";
        string fake;
        if (OperatingSystem.IsWindows())
        {
            fake = Path.Combine(dir, "claude.cmd");
        ...
        try
        {
            ClaudeHeadless.ResolveOverride = () => fake;
            ...
            if (OperatingSystem.IsWindows())
                ...
                Assert.Contains("ARGS=[-p][--output-format][json][--no-session-persistence][--model][haiku][--tools][]", got);
